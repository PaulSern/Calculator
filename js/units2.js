...
// "(1+(3-1)*4)^2*5+sin (5-3)/2"
// ["1+", ["3-1"], "*4"], "^2*5+", [Func, sin, "5-3"], "/2"]
// ["1+", [Minus, "3", "1"], "*4"], "^2*5+", [Func, sin, "5-3"], "/2"]
// [[Power, [Plus, "1", [Times, [Minus, "3", "1"], "4"]], "2"], "*5+", [Func, sin, "5-3"], "/2"]
// [Plus, [Times, [Power, [Plus, "1", [Times, [Minus, "3", "1"], "4"]], "2"], "5"], [Divide, [Func, "sin", [Minus, "5", "3"]], "2"]]
[Plus, 
	[Times, 
		[Power, 
			[Plus, 
...
function parenToArr(str) {
	...
	if (openParenPos !== -1) {		
		for (var i = openParenPos + 1, parenCount = 1, len = str.length; 0 < parenCount && i < len; i++) {		
			// Increment count when we find an open paren; decrement count when we find a closed one. 
			if (str[i] === "(") {
				parenCount++;
			} else if (str[i] === ")") {
				parenCount--;

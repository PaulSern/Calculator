(function (define, global) {
	...
	var prepareInputStr = (function () {
		...
		function normalizeWordTokens(str) {
			...
			while (i--) {
					...
					if (tokenObj.aliasRx) {
						...
						// Workaround for problem where negative signs were getting lost inside parentheses
						rx2 = RegExp(tokenObj.token + " ~", "gi"); // Look for negative signs that got accidently conveted to minus signs
						str = str.replace(rx2, tokenObj.token + " -"); // Change them back to negative signs
					}
				}
				return str;
			}
			...
			//------------------------------------------
			function normalizeConstants(str) {
				// TODO: Make this work for constants that aren't dimensionless
				var rx, key;
				for (key in CONSTANT) {
					if (key === 'e' || key === 'E') {
					    // Protect e in stuff like 1.34e-4
					    str = str.replace(/(!e!)/gi, '!x!');						
					} 
					rx = RegExp('\\b('+key+')\\b', 'gi');					
					str = str.replace(rx, CONSTANT[key].factor).replace(/!x!/gi, '!e!');					
				}
				return str;
			}			
			//------------------------------------------
			// Reformat str so it can be split into a special nested array
			function logPrepareInputStr(str, message) {
				if (global.prepLogOn || true) {
			...
			function prepareInputStr(str) {
				...
				logPrepareInputStr(str);
				// replace space between integers and fractions with "#" (high priority add), replace "/" fraction part with "&" (high priority divide for mixed numbers like this: 1 1/3 + 4 3/5
				...
				logPrepareInputStr(str);
				// put a "&" (high priority divide) betweeen numbers so stuff like this doesn't get broken by other rules: 1/2 meter in ft (but ignore stuff like 2 meter^2/2 meters)
				str = str.replace(/(^[\-]?[0-9]+|[^\-\^\{\\][\-]?[0-9]+)\/([\-]?[0-9])/g, "$1&$2");  // LH and RH negative signs: 1/2, -1/2, 1/-2, -1/-2 
				logPrepareInputStr(str);
				...
				logPrepareInputStr(str);
				// replace contants with numbers
				str = normalizeConstants(str);
				logPrepareInputStr(str, "normalizeConstants");
				...
				logPrepareInputStr(str, "normalizeWordTokens");
		...
		//-----------------------------------------
		// TODO: Don't break 1/2 meter in ft
	...
	var calcFromNestedParenStr = (function () {
		...
		*/
		...
		function calcFromNestedParenStr(nestedParenStr) {
				// Add times signs before and after
				nestedParenStr = nestedParenStr.replace(/(\d|[a-zA-Z])\(/, '$1*(').replace(/\)(\d|[a-zA-Z])/, ')*$1');
				// Add space after paren if followed by divide sign (fixes Issue 192 where numbers can get sucked into functions for stuff like sqrt(2)/5)
				nestedParenStr = nestedParenStr.replace(/\)(\/)/, ') /');
